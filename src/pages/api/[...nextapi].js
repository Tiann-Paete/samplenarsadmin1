  ...
  try {
    ...
    if (id) {
      const [product] = await db.query(
        `SELECT p.*, COALESCE(ps.quantity, 0) as current_stock 
         FROM products p 
         LEFT JOIN product_stocks ps ON p.id = ps.product_id 
...
async function handleAddStock(req, res) {
  ...
  try {
    ...
    const [existingStock] = await db.query(
      'SELECT * FROM product_stocks WHERE id = ? OR product_id = ?',
      [id, product_id]
    );
    ...
    if (existingStock.length > 0) {
      ...
      await db.query(updateSql, [newQuantity, existingStock[0].id]);
